...
////uint16_t prescaler[5] = {0, 1, 8, 64, 256, 1024};
//void timer0_config(uint16_t freq, uint8_t mode,uint16_t *prescaler[5], uint8_t Compare_Output_Mode){
				////POINTER TO ARRAY !!!!!111111111111
	...
	//case CTC_Mode:
			//switch(Compare_Output_Mode){
				...
				//break;
				//case 4: TCCR0A |= (0<<COM0A1) | (0<<COM0A0); //oc0a disabled
			//}
			////uint8_t TOP[256];  
			...
			//for (uint8_t k = 0; k < 6; k++){  // both k and j progressing at the same rate? each k needs to wait for 256 frequency calculations before moving to the subsequent prescaler
				//for (uint16_t j = 0; j < 4; j++){   //why not just j = 1, at 0 nothing happens => wasted time 
